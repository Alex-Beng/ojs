...
class Solution:
    def findCheapestPrice(self, n: int, flights: [[int]], src: int, dst: int, k: int) -> int:
        aja_mat = [[float('inf')]*n for _ in range(n)]
        tos = [dict() for _ in range(n)]
        for f in flights:
            tos[f[0]][f[1]] = f[2]
            aja_mat[f[0]][f[1]] = f[2]
        ...
        for tk in range(1, k+1):
            for c in range(n):
                dp[tk][c] = min( [dp[tk-1][mid]+aja_mat[mid][c] for mid in range(n) ] )
                # print(tk, c, dp[tk][c])
        
                
        ans = min( [dp[k][dst] for k in range(0, k+1)] )
