    ...
    def findWords(self, board: [[str]], words: [str]) -> [str]:
        ...
        for i in range(row):
            for j in range(col):
                cha = board[i][j]
                if cha in cha2coor:
                    cha2coor[cha].append((i, j))
                else:
                    cha2coor[cha] = [(i, j)]
        dr = [0,0,-1,1]
        dc = [-1,1,0,0]
        # @lru_cache(None)
        def dfs(r, c, left, been):
            if len(left) == 0:
                return True
            if board[r][c] == left[0]:
                if len(left)==1:
        ...
        for wd in words:
            wd_vec = get_vec(wd)
            if not subset(wd_vec, board_vec):
                continue
            
            head = wd[0]
            valid = False
            if head not in cha2coor:
                continue
            for coor in cha2coor[head]:
                # print(wd, coor)
                r, c = coor
                been = [[0]*col for _ in range(row)]
                been[r][c] = 1
                valid |= dfs(r, c, wd, been)
                been[r][c] = 0
                # print( valid, coor)
                if valid:
                    ans.append(wd)
